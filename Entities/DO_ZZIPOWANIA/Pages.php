...
class PagesModel 
{
	...
	public function createPagesResource(PagesResource $pages) : int
	{
		...
		$this->db->bind(':added', $pages->getAdded(), PDO::PARAM_STR);
		$this->db->bind(':deleted', $pages->getDeleted(), PDO::PARAM_STR);
	...
	private function getBindParamsDeclarationForUpdatePagesCollection(array $pagesCollectionContainer)
	{
		foreach ($pagesCollectionContainer as $index => $values) {
			$this->db->bind(":page_id_" . $index, $values['page_id']);
			$this->db->bind(":user_id_" . $index, $values['user_id']);
			$this->db->bind(":domain_" . $index, $values['domain']);
			$this->db->bind(":description_" . $index, $values['description']);
			$this->db->bind(":added_" . $index, $values['added']);
			$this->db->bind(":deleted_" . $index, $values['deleted']);
		}
	}
